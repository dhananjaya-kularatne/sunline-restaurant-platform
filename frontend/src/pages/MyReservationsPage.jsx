...
import { Calendar, Users, Clock, CheckCircle, ArrowRight, Lock, Plus, AlertCircle, X } from 'lucide-react';
        ...
        <div className="min-h-screen bg-transparent flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl w-full">
                <div className="bg-white rounded-2xl shadow-xl p-8 overflow-hidden h-fit border border-gray-100">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                        ...
                        <Link
                            to="/book-table"
                            className="bg-[#FF7F50] text-white px-6 py-3 rounded-lg font-bold hover:bg-[#e06b3f] transition-all flex items-center justify-center gap-2 shadow-md active:scale-[0.98] w-fit"
                        >
                    ...
                    {isLoadingReservations ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {reservations.map((res) => (
                                <div key={res.id} className="border border-gray-200 rounded-lg p-6 hover:border-[#FF7F50]/40 transition-all bg-white hover:shadow-md">
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 text-sm font-medium text-gray-600">
                                                <span className="flex items-center gap-1.5 bg-gray-50 px-2 py-1 rounded-md border border-gray-200">
                                    ...
                                    {res.specialRequest && (
                                        <div className="mb-5 p-3 bg-gray-50 rounded-lg border border-dashed border-gray-200 text-xs text-gray-500 italic">
                                            " {res.specialRequest} "
                                        </div>
                                    )}
                                    ...
                                    {res.status === 'PENDING' && (
                                        <button
                                            ...
                                            className="w-full mt-2 py-2.5 rounded-lg border border-red-200 text-red-500 text-sm font-bold hover:bg-red-50 hover:border-red-300 transition-all"
                                        >
                                            Cancel Reservation
                                        </button>
                                    )}
                        ...
                        <div className="text-center py-20 px-4 bg-transparent rounded-lg border-2 border-dashed border-gray-200">
                            ...
                            <h3 className="text-xl font-bold text-gray-800 mb-2">No reservations yet</h3>
                            <p className="text-gray-500 mb-8 max-w-xs mx-auto">
                                You haven't reserved any tables yet. Start your dining experience with us today!
                            </p>
                            <Link
                                to="/book-table"
                                className="inline-flex items-center gap-2 bg-[#FF7F50] text-white px-6 py-3 rounded-lg font-bold hover:bg-[#e06b3f] transition-all shadow-md active:scale-[0.98]"
                            >
            ...
            {cancelModal.show && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl max-w-sm w-full p-8 shadow-2xl border border-gray-100">
                        <div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-6">
                            <Calendar size={32} />
                        </div>
                        <h3 className="text-2xl font-bold text-gray-800 mb-3 text-center">Cancel Reservation?</h3>
                        ...
                        <div className="flex flex-col gap-3">
                            <button
                                ...
                                className="w-full py-3 px-4 rounded-lg font-bold text-white bg-red-500 hover:bg-red-600 transition-all shadow-md active:scale-[0.98]"
                            >
                                Yes, Cancel Reservation
                            </button>
                            <button
                                onClick={() => setCancelModal({ show: false, reservationId: null })}
                                className="w-full py-3 px-4 rounded-lg font-bold text-gray-600 border border-gray-100 hover:bg-gray-50 transition-all"
                            >
                                No, Keep it
                            </button>
                        </div>
